...
int main(int argc, const char * argv[]) {
    ...
    for(i=0;i<nParameters;i++){
        ...
        if(i==0){
        sprintf(widthFilename,"./a_%Lf_m_%d_n_%d_widths.out",a0[0],m,n);
...
void ResonanceWidths(long double a, long double b, long double x0, long double y0, int m, int n, int updown,long double *width){
    ...
    long double stepsize = 1/nYsteps,yf,omega[200][2],omega0,dom,omax,omin,y=y0-stepsize*100;
    char filename[60]="./",astr[60],bstr[60],mstr[60],nstr[60],updownstr[60];
    FILE *fl;
    
    long double yLow=y, yHigh=y0+stepsize*100;
    ...
    for(i=0;i<200;i++){
        ...
        if(omega[i][updown] > UNDEFINED){
            fprintf(fl,"%21.17Lf %21.17Lf\n",y, omega[i][updown]);
            if(  fabsl(omega[i][updown]-omega0) > 1.0e-4L  &&  i<100  ){
                yLow = y; error=1;
                //printf("Bottom bound was %Lf\n",yLow);
            }
            if(  fabsl(omega[i][updown]-omega0) > 1.0e-4L  &&  i>100 && error != 0){
                yHigh = y;
                error+=1;
