#include <math.h>
#define SHFT(a,b,c,d) (a)=(b);(b)=(c);(c)=(d);
#define ITMAX 100
#define EPS LDBL_EPSILON /* machine precision .. used in ZBRENT */
#define ZEPS EPS /* a small number; used in brent */
#define TINY EPS /* really small number; used in mnbrac */
#define tol 3.2e-10 
/* precision of minima found with brent; should be sqrt of LDBL_EPSILON */
#define CGOLD 0.3819660112501051517954131656343618822796908201942
#define ERROR 123456789.0

long double brent(long double, long double, long double, 
		  long double (*)(long double), long double *);

long double brent(long double ax, long double bx, long double cx, 
		  long double (*f)(long double), long double *xmin)
{
  int iter;
  long double a,b,d,etemp,fu,fv,fw,fx,p,q,r,tol1,tol2,u,v,w,x,xm,e=0.0;

  a=(ax < cx ? ax : cx);
  b=(ax > cx ? ax : cx);
  x=w=v=bx;
  fw=fv=fx=(*f)(x); 
  for (iter=1; iter<=ITMAX; iter++) {
    xm=0.5*(a+b);
    tol2= 2.0*(tol1=tol*fabsl(x)+ZEPS);
    if (fabsl(x-xm) <= (tol2-0.5*(b-a))) {
      *xmin = x;
      return fx;
    }
    if (fabsl(e) > tol1) {
      r = (x-w)*(fx-fv);
      q = (x-v)*(fx-fw);
      p = (x-v)*q - (x-w)*r;
      q = 2.0*(q-r);
      if (q > 0.0) p = -p;
      q = fabsl(q);
      etemp=e;
      e=d;
      if (fabsl(p) >= fabsl(0.5*q*etemp) || p <= q*(a-x) || p >= q*(b-x))
	d = CGOLD*(e=(x >= xm ? a-x : b-x));
      else {
	d = p/q;
	u = x+d;
	if (u-a < tol2 || b-u < tol2)
	  d = SIGN(tol1,xm-x);
      }
    } else {
      d = CGOLD*(e=(x >= xm ? a-x : b-x));
    }
    u = (fabsl(d) >= tol1 ? x+d : x+SIGN(tol1,d));
    fu = (*f)(u); 
    if (fu <= fx) {
      if (u >= x) a = x; else b = x;
      SHFT(v,w,x,u);
      SHFT(fv,fw,fx,fu);
    } else {
      if (u < x) a = u; else b = u;
      if (fu <= fw || w == x) {
	v=w;
	w=u;
	fv=fw;
	fw=fu;
      } else if (fu <= fv || v == x || v == w) {
	v = u;
	fv = fu;
      }
    }
  }
  *xmin = x;
  return ERROR;
}

