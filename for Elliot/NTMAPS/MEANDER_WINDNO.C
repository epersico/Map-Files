...
const char *infilename="meander-windno.in", 
  *outfilename="/tmp/meander-windno.out";
...
main(int argc, char **argv)
{
  ...
  int i, ndom, nomax, nomin, ncutoff, skip;
  long double dom, omega, omax, omin;
  FILE *fl, *flbc;
  char dummy[200];
  ...
  while ((a<=amax) && (b<=bmax))
      ...
      while (b<=bmax)
	{
	  if (skip<speedup) {
	    ...
	    fprintf(fl, 
		    " %15.12Lf %15.12Lf %15.8Lg %10d %10.3Le %10d %10.7Lf %10d %10.7Lf %10d\n",
		    a, b, UNDEFINED, 0, UNDEFINED, 0, 0.0L, 0, 0.0L, 0);	  
	  if (astep<0.0) b=bmax+1.0L;
	  else b+=bstep;
	}

      if (astep<0.0) {
	fscanf(flbc,"%Lf %Lf", &a, &b);
	fgets(dummy, 200, flbc);
      }
      else {
	a+=astep;
...
void makeinitials(void)      /* read initial values */
{
  ...
  if (astep<0.0) fscanf(fl,"%s", data_file);         /* filename for astep<0.0 */
