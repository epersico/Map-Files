...
void maincalc() 
{
    long double braca, bracb, bracc, sol[2*xbranch+1][2], checker, bracd;
    int xcount, i;
...
/*	    bracd=(sol[xcount/2][0]-sol[xcount/2-1][0]<sol[xcount/2+1][0]-sol[xcount/2][0] ? 
	    -sol[xcount/2][0]+sol[xcount/2-1][0] : sol[xcount/2+1][0]-sol[xcount/2][0])/3.0L; */
	    if (fabsl(bracd)<bracdistmin) bracd=bracd/fabsl(bracd)*bracdistmin;
	    if (fabsl(bracd)>bracdist) bracd=bracd/fabsl(bracd)*bracdist;
	    braca-=bracd/2.0L;
	}
	else braca=sol[xcount/2][0];
	bracb=braca-bracd;
      }
	  ...
	  sol[xcount][1]=brent(braca, bracb, bracc, rootbynegfunc, sln, qq[qpi], pp[qpi], 
			       &sol[xcount][0]);
	  checker=rootbyfunc(sol[xcount][0]+bracd, sln, qq[qpi], pp[qpi]);
      }
