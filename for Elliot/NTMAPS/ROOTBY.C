...
const char *infilename="rootby.in";
...
long double pi2;
long double omega, a0, a00, b0, b00, ymin, ymax, ystep, y00, bxx;
int sln, slnmin, slnmax, qpi, qp_no, bres, bresx;
...
main(int argc, char **argv)
{
  ...
  for (sln=slnmin;sln<=slnmax;sln++) {
      ...
      for (y00=ymin; y00<=ymax; y00+=ystep) maincalc();
...
void makeinitials(void)      /* read and adjust initial values */
{
  ...
  fscanf(fl,"%d %d", &bres, &bresx);  NL(fl, dummy);     /* # of brackets, # used */
...
void maincalc(void) 
{
    long double braku[bres+1], brakl[bres+1];
    int brakno=(bresx==0 ? bres : bresx), i;

    zbrak(*fiterfunc, sln, qq[qpi], pp[qpi], b0, bxx, bres, &brakl[0], &braku[0], &brakno);
    if (brakno>0) fprintf(fl1, "%25.18Lf%25.18Lf%15d\n", 
			  zbrent(*fiterfunc, sln, qq[qpi], pp[qpi], brakl[brakno], braku[brakno]),
			  y00, brakno);
    if (bresx==0) for (i=1; i<=brakno-1; i++) 
	fprintf(fl1, "%25.18Lf%25.18Lf\n", 
		zbrent(*fiterfunc, sln, qq[qpi], pp[qpi], brakl[i], braku[i]),y00);
}
...
long double fiter2funcoe0(long double b, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  register unsigned long int i, j, jj, qf, pf;
  register long double xxtmp, yytmp;

  pf=ppp; if (sss==1) qf=qqq-1; else qf=qqq+1;
  xxtmp=findx0(a0, y00, sss); yytmp=y00; jj=itstime2mod; j=(unsigned long int)((double)(jj)*(((double)(pf))/((double)(qf))));
  for (i=1;i<=ppp/2;i++) {
...
long double fiter2funcoe1(long double b, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  register unsigned long int i, j, jj, qf, pf;
  register long double xxtmp, yytmp;
...
long double fiter2funcoo0(long double b, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  register unsigned long int i, j, jj, qf, pf;
  register long double xxtmp, yytmp;
...
long double fiter2funcoo1(long double b, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  register unsigned long int i, j, jj, qf, pf;
  register long double xxtmp, yytmp;
...
long double fiter2funceo0(long double b, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  register unsigned long int i, j, jj, qf, pf;
  register long double xxtmp, yytmp;
...
long double fiter2funceo1(long double b, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  register unsigned long int i, j, jj, qf, pf;
  register long double xxtmp, yytmp;
