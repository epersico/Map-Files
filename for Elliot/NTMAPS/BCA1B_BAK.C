...
void maincalc() 
{
    long double braca, bracb, bracc, sol[10][2], nsol[10][2], /*sol[2*xbranch+2][2], nsol[2*xbranch+2][2], */
	checker, bracd, mindist;
    int xcount, i, j, jold;
	...
	j=0; jold=0; for (i=0;i<=xcount;i+=2) {
	    ...
	    nsol[j][1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], 
				   &nsol[j][0]);
	    if (fabsl(fabsl(nsol[j][1])-ERROR)>SMALL) 
	    ...
	    if (j==jold+2) j-=1;
		  ...
		  if (rootbynegfunc(nsol[jold][0], sln, qq[qpi], pp[qpi])>checker &&
			...
			|| fabsl(fabsl(nsol[j-2][1])-ERROR)<SMALL) 
...
f(*bx)<f(*cx), where *ax and *cxmust not be exceeded */
{
  ...
  if ((*ax-*bx)*(*bx-*cx)<0) *bx=(*ax + *cx)/2.0L; /* need *bx is inside bracket */
  ...
  *cx = (*bx) + GOLD*(*bx-*ax);
  ...
  for (i=1;i<=NTRY;i++) {
      ...
      c0=b0;
      if (i==NTRY) {
	  *ax=a0; *bx=a0+distx/2.0L; *cx=a0+distx;     /* try again, from ignored end of bracket */ 
	  fa = (*func)(*ax, par1, par2, par3); 
	  fb = (*func)(*bx, par1, par2, par3);
	  fc = (*func)(*cx, par1, par2, par3);
	  if (fa>fb && fc>fb) return 1;                /* bracket inside start interval */
	  ...
	  *bx=*cx; fb=fc;
      ...
      if (i<NTRY && fb>fa) {
	  *ax=(a0+c0)/2.0L; *bx=*ax+distx/2.0L; *cx=*ax+distx;   /* last try */ 
	  fa = (*func)(*ax, par1, par2, par3); 
	  fb = (*func)(*bx, par1, par2, par3);
	  fc = (*func)(*cx, par1, par2, par3);
	  if (fa>fb && fc>fb) return 1;                /* bracket inside start interval */
