...
/*    long double braca, bracb, bracc, sol[10][2], nsol[10][2], sol[2*xbranch+2][2], nsol[2*xbranch+2][2], checker, bracd, mindist, intervals=INTERVALS; */
	...
	if (rootbyfunc(boundl, sln, qq[qpi], pp[qpi])>=0.0L) {
	    ...
	    if (nsol[0][1]<0.0L)
	      nsol[0][1]=-rootbyfunc(boundl, sln, qq[qpi], pp[qpi]);
	}
	...
	if (rootbyfunc(boundu, sln, qq[qpi], pp[qpi])>=0.0L) {
	    ...
	    if (nsol[xcount+1][1]<0.0L)
	      nsol[xcount+1][1]=-rootbyfunc(boundu, sln, qq[qpi], pp[qpi]);
	}
	...
	i=1; while (i<=xbranch && xcount+2<=solno){
		...
		if (okbrac==1 && rootbyfunc(tmp[0], sln, qq[qpi], pp[qpi])<0.0L) {
		    ...
		    tmp[0]=(j>=3? nsol[j-1][0]: boundl);
		    if (tmp[0]<nsol[j][0] && j+2<=xcount) {
	...
	fprintf(fl1, "%25.18Lf%25.18Lf%25.18Lf%10d\n", 
		a0, nsol[i][1], nsol[i][0], (i<1 || i>xcount ? 100+xcount: 2*(i%2)-1));  
    ...
    for (a0=a0min+a0step; a0<=a0max; a0+=a0step) {
      ...
      /* reestimate bounds on domain */
      bracb=bracdistmin;
      tmp[0]=boundl-bracb; tmp[1]=boundl+bracb; okbrac=0;
      while (okbrac==0 && bracb<=bracdist) {
	if (rootbyfunc(tmp[0], sln, qq[qpi], pp[qpi])>=0.0L) { 
      ...
      if (!okbrac) printf("Warning, lower bound lost at a=%Lf.\n", a0);
      nsol[0][1]=-1.0L;
      for (i=1;i<=7;i++) {
	boundl=(tmp[0]+tmp[1])/2.0L;
	if ((braca=rootbyfunc(boundl, sln, qq[qpi], pp[qpi]))>=0.0L) tmp[0]=boundl;
      ...
      while (okbrac==0 && bracb<=bracdist) {
	if (rootbyfunc(tmp[1], sln, qq[qpi], pp[qpi])>=0.0L) { 
      ...
      if (!okbrac) printf("Warning, upper bound lost at a=%Lf.\n", a0);
