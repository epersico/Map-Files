...
const char *infilename="b_iter.in";
...
long double pi2;
long double omega, a0, a00, b0, b00, y00, bxx, bres, b;
...
void makeinitials(void)      /* read and adjust initial values */
{
  ...
  fscanf(fl,"%Lf", &y00);  NL(fl, dummy);              /* y0 */
  fscanf(fl,"%Lf", &b00);  NL(fl, dummy);              /* start b */
  fscanf(fl,"%Lf", &bxx);  NL(fl, dummy);              /* max b */
  fscanf(fl,"%Lg", &bres);  NL(fl, dummy);             /* b resolution */
  ...
  if (bres<SMALL) bres=bxx-b00+SMALL;
  fclose(fl);
}
...
void prepoutfiles(void)
{
  ...
  fprintf(fl1,"# a0:      %10.7Lf , y0:      %10.7Lf \n", a0, y00); 
  fprintf(fl1,"# b0:      %10.7Lf , bx:      %10.7Lf \n", b0, bxx); 
  fprintf(fl1,"# bres:    %10.7Lf , mod:     %10u \n", bres, itstime2mod); 
  fprintf(fl1,"# %18s%25s%25s%25s\n\n", "b", "f_iter1", "f_iter2", "f_iter2b");
...
long double fiter1funceven1(long double yy0, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  ...
  return sinl(pi2*(xxtmp-a0/2.0L*(1.0L-yytmp*yytmp)));
}
...
long double fiter2funcoe0(long double yy0, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  ...
  xxtmp=findx0(a0, yy0, sss); yytmp=yy0; jj=itstime2mod; j=jj*pf/qf;
  for (i=1;i<=ppp/2;i++) {
    ntmapstep(xxtmp, yytmp, a0, b); 
...
long double fiter2funcoe1(long double yy0, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  ...
  xxtmp-=((long double)(qf/2-(jj-itstime2mod)));
  if (sss==3) return xxtmp-a0/2.0L*(1.0L-yytmp*yytmp)-0.5L;
...
long double fiter2funcoo0(long double yy0, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  ...
  xxtmp-=((long double)(qf/2-(jj-itstime2mod)));
  if (sss==1) return xxtmp-a0/2.0L*(1.0L-yytmp*yytmp)-0.5L;
  else return xxtmp-a0/2.0L*(1.0L-yytmp*yytmp);
}
...
long double fiter2funceo0(long double yy0, int sss, unsigned long int qqq, unsigned long int ppp) 
{
  ...
  xxtmp-=((long double)(qf/2-(jj-itstime2mod)));
  if (sss==1) return xxtmp-a0/2.0L*(1.0L-yytmp*yytmp);
