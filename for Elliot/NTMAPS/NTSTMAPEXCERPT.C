...
const char *infilename="ntstmapexcerpt.in";
char outfilename[160];

long double   a, b, jj0[200], th0[200], thstart, thend, jjstart, jjend;
long double n, skip;
int xy_no;
...
main(int argc, char **argv)
{
  ...
  /* forward iteration */    
      ...
      fprintf(fl, "#%15s %20s %20s %15s\n","step", "x", "y", "wind");
      fprintf(fl, " %15u %20.15Lf %20.15Lf %20.15Lf\n", 
	      0L, th, jj, thlift-th00);
    
      /* main loop */
      for (ck=1.0L; ck<=n; ck++)
	{
	  mapstep(&jj,&th, &thlift);
	  if (fabsl((floorl(ck/skip))*skip-ck)<1.0e-17L)
	      if (th>thstart && th<thend && jj>jjstart && jj<jjend)
	      fprintf(fl, " %15.0Lf %20.15Lf %20.15Lf %20.15Lf\n", 
		      ck, th, jj, (thlift-th00)/ck);

	  /* progress report */
	  pr++;
	  if (floor(40.0L*(pr+1.0L)/prt)
	      >floor(40.0L*pr/prt))
	    {
...
void makeinitials(void)      /* read initial values */
{
  ...
  fscanf(fl,"%Lf %Lf", &thstart, &thend); NL(fl, dummy);       /* th range */
  fscanf(fl,"%Lf %Lf", &jjstart, &jjend); NL(fl, dummy);       /* jj range */
  fscanf(fl,"%Lf %Lf", &n, &skip); NL(fl, dummy);    /* # steps, skip output */
