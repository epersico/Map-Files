...
const char *infilename="meander-windno.in", 
  *outfilename="/tmp/meander-windno-shinob.out";
...
main(int argc, char **argv)
{
  ...
  long double dom=0.0L, omega=0.0L, omax=0.0L, omin=0.0L, btmp;
  ...
  while ((a<=amax) && (b<=bmax))
      ...
      while (b<=bmax)
	{
	  if (skip<speedup) {
		...
		btmp=b; solutionq=solutionq0; b=b-bstep;
		for (i=1;i<=REFINE;i++) {
		    b+=bstep/((long double)(REFINE));
		    find_om(shino(a, sms), 0.0L, &omega, &ncutoff, &dom, &ndom, 
			    &omax, &nomax, &omin, &nomin);
