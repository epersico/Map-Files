...
main(int argc, char **argv)
{
  ...
  long double y0, omx, omold, dir, dsl, dom, omega, omax, omin;
  FILE *fl, *flx;
  char dummy[200];
  ...
  for (abloop=0;abloop<nab;abloop++)
      ...
      for (dsl=dslmin;dsl<dslmax;dsl+=dslstep) 
	{
	  ...
	  while (y0<=ymax)
	    {
	      if (skip<speedup) {
		find_om(symmline(y0,sl)+dsl, y0, &omega, &ncutoff, &dom, 
			&ndom, &omax, &nomax, &omin, &nomin);
		if (omega<UNDEFINED+1.0) {
		  ...
		  if ((fabs(omega-omx)>cutoff) && (dir*(omega-omold)<0.0)) {
		    fprintf(flx, " %10.7f %20.17f %20.13f %20.17f %8d %10.3e %8d %10.3g %8d %10.3g %8d\n", 
			    dsl, y0, omega, omega-omx, ncutoff, dom, ndom, 
			    omax, nomax, omin, nomin);
		    dir *= -1.0L;
		    omx = omega;
		    xcount++; 
		...
		fprintf(fl, " %10.7Lf %20.17Lf %20.13Lf %8d %10.3Le %8d %10.3Lg %8d %10.3Lg %8d\n", 
			dsl, y0, omega, ncutoff, dom, ndom, omax, nomax, omin, nomin);
	  }
	      else
		fprintf(fl, " %10.7Lf %20.17Lf %20.13Lf %8d %10.3Le %8d %10.3Lg %8d %10.Lg %8d\n", 
			dsl, y0, UNDEFINED, 0, UNDEFINED, 0, 0.0, 0, 0.0, 0);	  
