...
#define ntmapstep(x,y,a,b) {y-=(b)*sinl(pi2*(x)); x+=(a)*(1.0L-(y)*(y));}
...
long double pi2;
long double omega, a0, a00, b0, b00, y00, yxx, yres;
...
main(int argc, char **argv)
{
  ...
  for (sln=slnmin;sln<=slnmax;sln++) {
	  ...
	  tmp=fiter2func(y,sln,qq[qpi],pp[qpi]); smoothen(tmp,tmp2);
	  fprintf(fl1,"%20.13Lf%25.15Lg%25.15Lg%25.15Lg\n", 
	     y, fiter1func(y,sln,qq[qpi],pp[qpi]), tanhl(tmp), tmp2);
      }
...
void prepoutfiles(void)
{
  ...
  fprintf(fl1,"# yres:    %10.7Lf , mod:     %10u \n", yres, itstime2mod); 
