...
const char *infilename="f_itery.in.in", outfilename[4][180];
...
void prepoutfiles(void)
{
  ...
  if (outfiles>2) {
    ...
    fprintf(fl3,"# brakl0:  %10.7Lf , braku0:  %10.7Lf , brakn:    %10d\n", 
	    brakl0, braku0, brakn); 
    fprintf(fl3,"# acc:     %10.3Le , lacc:      %10.3Le , eqd:     %10.3Le\n", acc, lacc, eqd); 
  }
}
...
void assignshino(int ss, long double *xval, long double *yval)
{
  ...
  /* x modulo 1 */
  while (*xval> 0.995L) *xval=*xval-1.0L;
  while (*xval< -0.005L) *xval=*xval+1.0L;
}
...
int findshinofp(int shp, long double *shx, long double *shy, long double *shlx, long double *shly)
{
    ...
    *shly=exp(log(fabsl(dyy))/((long double) (10)));
    ...
    while (fabsl(xx-xx0)<=fabsl(dxx)+acc && fabsl(yy-yy0)<=fabsl(dyy)+acc && fabsl(yy-yy0)>acc && 
	(fabsl(*shlx-exp(log(fabsl(xx-xx0))/((long double) (j))))>lacc ||
	...
	*shly=exp(log(fabsl(dyy))/((long double) (j)));
    ...
    else if (fabsl(xx-xx0)<=fabsl(dxx)+acc && fabsl(yy-yy0)<=fabsl(dyy)+acc && j<ROOT_NO) {
	*shlx=exp(log(fabsl(xx-xx0))/((long double) (j)));
	*shly=exp(log(fabsl(yy-yy0))/((long double) (j)));
	if (xx>xx0) xx=xx0+pow(*shlx,((long double) (j)))/(1.0L-(*shlx));
	...
	if (fabsl(yy-yy0)<acc) {*shx=xx; *shy=yy; return 1;} else {
	    while (fabsl(xx-xx0)<=fabsl(dxx)+acc && fabsl(yy-yy0)<=fabsl(dyy)+acc 
		   && fabsl(yy-yy0)>acc && j<ROOT_NO) {
		dxx=xx-xx0; dyy=yy-yy0;
		*shlx=exp(log(fabsl(dxx))/((long double) (j)));
		*shly=exp(log(fabsl(dyy))/((long double) (j)));
		xx0=xx; yy0=yy;
