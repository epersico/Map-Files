...
#define IPOW3(a) ((int)(pow(3.0L, (long double)(a))+0.5L)) 
#define ERROR 123.456789L
#define UNDEFINED -123456.789L

const char *infilename="bca2.in";
...
void prepoutfiles(void)
{
  ...
  fprintf(fl1,"# codes: 1 max, -1 min, 2 leaving b-range, -2 entering b-range\n");
  fprintf(fl1,"#        0 pseudo-extr. outside b-range\n");
  fprintf(fl1,"#        (n*1000+m)*100+N boundaries - found after n+m iterations/ bound N extrema \n\n");
}
...
/*    long double braca, bracb, bracc, sol[10][2], nsol[10][2], sol[2*xbranch+2][2], nsol[2*xbranch+2][2], checker, bracd, mindist, intervals=INTERVALS; */
    ...
    long double braca, bracb, bracc, sol[IPOW3(xbranch)+3][3], nsol[IPOW3(xbranch)+3][3], 
	checker, inters=INTERVALS, boundl, boundu, tmp[2], outofbounds; 
	...
	xcount=1;
	sol[xcount][0]=0.0L; sol[xcount][1]=0.0L; sol[xcount][2]=1.0L;
	nsol[xcount][0]=0.0L; nsol[xcount][1]=0.0L; nsol[xcount][2]=1.0L;
	...
	nsol[1][2]=((double)(esti[1]*100));
	outofbounds=(boundu-boundl)/inters/10.0L;


	/* find first maximum */
	braca=boundl-outofbounds; bracc=boundu+outofbounds; bracb=(boundl+boundu)/2.0L;
	if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))>=1) {
   	    nsol[xcount][1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], 
				   &nsol[xcount][0]);
	    nsol[xcount][2]=((double)(okbrac));
	    if (okbrac==2) {
		if (rootbyfunc(nsol[xcount][0]-outofbounds, sln, qq[qpi], pp[qpi])>=5.0L) 
		    nsol[xcount][2]=-nsol[xcount][2];
	        else if (rootbyfunc(nsol[xcount][0]+outofbounds, sln, qq[qpi], pp[qpi])<5.0L) 
		    nsol[xcount][2]=1.0L;
	    }	    
	}
	...
	/* find higher branch extrema */
	i=1; while (i<=xbranch && xcount+2<=solno){
	    j=1; while (j<=xcount+2 && xcount+2<=solno){
	        braca=(j>=3? nsol[j-2][0]:boundl-outofbounds);     /* find maxima */ 
	        bracc=(j<=xcount? nsol[j][0]: boundu+outofbounds);
		bracb=braca-1.0L; 
		if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))>=1)
		    tmp[1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &tmp[0]);

		if (j>=3)    /* solution "same" as neighbors to current resolution */ 
		    if (fabsl(nsol[j-2][1]-tmp[1])<10.0L*(bxx-b0)/((long double) (bres))) 
			if (fabsl(nsol[j-2][0]-tmp[0])<10.0L*fabsl(tmp[0])/tmp[1]*(bxx-b0)/((long double) (bres))) 
			    okbrac=0;
		if (j<=xcount)
		    if (fabsl(nsol[j][1]-tmp[1])<10.0L*(bxx-b0)/((long double) (bres))) 
			if (fabsl(nsol[j][0]-tmp[0])<10.0L*fabsl(tmp[0])/tmp[1]*(bxx-b0)/((long double) (bres))) 
			    okbrac=0;

		
		if (okbrac>=1 && rootbyfunc(tmp[0], sln, qq[qpi], pp[qpi])<0.0L) {
		    for (k=xcount+1; k>=j; k--) {nsol[k+2][0]=nsol[k][0]; nsol[k+2][1]=nsol[k][1]; nsol[k+2][2]=nsol[k][2];}
		    nsol[j][0]=tmp[0]; nsol[j][1]=tmp[1]; nsol[j][2]=((double)(okbrac));
		    if (okbrac==2) {
			if (rootbyfunc(nsol[j][0]-outofbounds, sln, qq[qpi], pp[qpi])>=5.0L) 
			    nsol[j][2]=-nsol[j][2];
			else if (rootbyfunc(nsol[j][0]+outofbounds, sln, qq[qpi], pp[qpi])<5.0L) 
			    nsol[j][2]=1.0L;
		    }	    
		    xcount+=2;
		    if (nsol[j][0]<boundl) {boundl=nsol[j][0]; nsol[0][0]=boundl;} /* adjust lower bound if necessary */
		    if (nsol[j][0]>boundu) {boundu=nsol[j][0]; nsol[xcount+1][0]=boundu;} /* adj. upper bound if nec. */

		    if (nsol[j-1][0]<=nsol[j][0] && j+2<=xcount) { /* find minima */
			if (nsol[j][2]-nsol[j+2][2]<3.99L) {
				...
				nsol[j+1][1]=brent(braca, bracb, bracc, rootbynegfunc, sln, qq[qpi], pp[qpi], 
						   &nsol[j+1][0]);
				nsol[j+1][2]=-((double)(okbrac));
			    } else {
				printf("(%d) Warning: No right minimum between start maxima %d...\n", sln, j+1);
				nsol[j+1][0]=nsol[j][0];
				nsol[j+1][1]=nsol[j][1];			    
				nsol[j+1][2]=-3.0L;
			    }
			} else {
			    nsol[j+1][0]=(nsol[j][0]+nsol[j+2][0])/2.0L;
			    nsol[j+1][1]=(nsol[j][1]+nsol[j+2][1])/2.0L;			    
			    nsol[j+1][2]=0.0L;
			}			
		    }
		    else if (nsol[j-1][0]>=nsol[j][0] && j>=3) {
		        nsol[j+1][0]=nsol[j-1][0]; nsol[j+1][1]=nsol[j-1][1]; nsol[j+1][2]=nsol[j-1][2];
			if (nsol[j-2][2]-nsol[j][2]<3.99L) {
				...
				printf("(%d) Warning: No left minimum between start maxima %d...\n", sln, j-1);
				nsol[j-1][0]=nsol[j-2][0];
				nsol[j-1][1]=nsol[j-2][1];			    
				nsol[j+1][2]=-3.0L;
			    }
			} else {
			    nsol[j-1][0]=(nsol[j][0]+nsol[j-2][0])/2.0L;
			...
			printf("(%d) Warning: No minimum between start maxima %d...\n", sln , j);
			nsol[j+1][0]=nsol[j][0];
			nsol[j+1][1]=nsol[j][1];			    
			nsol[j+1][2]=-10.0L;
		    }
	...
	fprintf(fl1, "%25.18Lf%25.18Lf%25.18Lf%15d\n", 
		a0, nsol[i][1], nsol[i][0], (i<1 || i>xcount ? esti[i/(xcount+1)]*100+xcount: ((int)(nsol[i][2]))));  
	sol[i][0]=nsol[i][0]; sol[i][1]=nsol[i][1]; sol[i][2]=nsol[i][2];
    }
    fprintf(fl1,"\n");
      ...
      esti[1]*=1000; while (fabsl(tmp[0]-tmp[1])>bsharp && esti[1]%1000<INTERVALS)
	{
      ...
      boundu=tmp[0]; nsol[xcount+1][0]=boundu;
      if (nsol[xcount+1][1]<0.0L) nsol[xcount+1][1]=-rootbyfunc(boundu, sln, qq[qpi], pp[qpi]);
      nsol[xcount+1][2]=((double)(esti[1]*100));
      outofbounds=(boundu-boundl)/inters/10.0L;


      /* find new extrema from old ones */
      i=1; j=1; while (j<xcount && xcount+2<=solno){
	braca=(j>=3? nsol[j-1][0]:boundl-outofbounds);   /* find maxima */
	bracb=sol[j][0]; 
	bracc=sol[j+1][0];
	if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))>=1)
	  tmp[1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &tmp[0]);
	...
	if (okbrac>=1 && rootbyfunc(tmp[0], sln, qq[qpi], pp[qpi])<0.0L) {
	  ...
	  if (nsol[j-1][0]<=nsol[j][0] && j+2<=xcount) { /* find minima */
		...
		printf("(%d) Warning: No right minimum between maxima %d at a0=%Lf...\n", sln, j+1, a0);
		nsol[j+1][0]=nsol[j][0];
		nsol[j+1][1]=nsol[j][1];			    
		nsol[j+1][2]=-3.0L;
	      }
	    } else {
	      nsol[j+1][0]=(nsol[j][0]+sol[j+2][0])/2.0L;
	      nsol[j+1][1]=(nsol[j][1]+sol[j+2][1])/2.0L;			    
	      nsol[j+1][2]=0.0L;
	    }	
	  } else if (nsol[j-1][0]>=nsol[j][0] && j>=3) {
	    nsol[j+1][0]=nsol[j-1][0]; nsol[j+1][1]=nsol[j-1][1]; nsol[j+1][2]=nsol[j-1][2];
		...
		printf("(%d) Warning: No left minimum between maxima %d at a0=%Lf...\n", sln, j-1, a0);
		nsol[j-1][0]=nsol[j-2][0];
		nsol[j-1][1]=nsol[j-2][1];			    
		nsol[j-1][2]=-3.0L;
	      }
	    ...
	    printf("(%d) Warning: No minimum between maxima %d... at a0=%Lf\n", sln , j, a0);
			nsol[j+1][0]=nsol[j][0];
			nsol[j+1][1]=nsol[j][1];			    
			nsol[j+1][2]=-10.0L;
		    }
	...
	j+=2;}
      ...
      if (j==xcount) {	      
	  braca=nsol[j-1][0]; bracb=sol[j][0]; bracc=boundu+outofbounds;
	  if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))>=1)
	      tmp[1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &tmp[0]);
	  if (okbrac>=1 && rootbyfunc(tmp[0], sln, qq[qpi], pp[qpi])<0.0L) 
	    { 
	      nsol[j][0]=tmp[0]; nsol[j][1]=tmp[1]; nsol[j][2]=okbrac;
	      if (okbrac==2) {
		  if (rootbyfunc(nsol[j][0]-outofbounds, sln, qq[qpi], pp[qpi])>=5.0L) 
		      nsol[j][2]=-nsol[j][2];
		  else if (rootbyfunc(nsol[j][0]+outofbounds, sln, qq[qpi], pp[qpi])<5.0L) 
      ...
      /* find new extrema branches in between */
      i++; while (i<=xbranch && xcount+2<=solno){
	  j=1; while (j<=xcount+2 && xcount+2<=solno){
	      braca=(j>=3? nsol[j-2][0]: (nsol[j][0]==boundl? boundl: boundl-outofbounds));  /* find maxima */
	      bracc=(j<=xcount? nsol[j][0]: boundu+outofbounds);
	      bracb=braca-1.0L; 
	      if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))>=1)
		  tmp[1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &tmp[0]);
      ...
      j=0; for (i=0; i<=xcount+1; i++) {
	  if (j>=2) if (nsol[j-2][2]<-2.01) j-=2;             /* throw out crappy values */
	  nsol[j][0]=nsol[i][0]; nsol[j][1]=nsol[j][1]; nsol[j][2]=nsol[i][2];	    
	  j++;	
      }
      fprintf(fl1,"\n");
      nsol[0][2]+=((double)(xcount)); nsol[j-1][2]+=((double)(xcount)); xcount=j-2; 
      for (i=0; i<=xcount+1; i++) {
	  fprintf(fl1, "%25.18Lf%25.18Lf%25.18Lf%15d\n", 
		  a0, nsol[i][1], nsol[i][0], (i<1 || i>xcount ? esti[i/(xcount+1)]*100+xcount: ((int)(nsol[i][2]))));   
	  sol[i][0]=nsol[i][0]; sol[i][1]=nsol[i][1]; sol[i][2]=nsol[i][2];
...
long double rootbyfunc(long double y, int sss, unsigned long int qqq, unsigned long int ppp)
{
    ...
    else return 5.0L+fabsl(sqrtl(1.0L-omega/a0)-fabsl(y)); 
/*    else return fabsl(y-10.0L*sqrtl(1.0L-omega/a0)); */
}
...
return values: 1 - minimum found, 2 - boundary found, 0 - bracket size too small, 
-i - no convergence after i attempts, -1 - nothing worked */
{
  ...
  if (fa>fb && fc>fb) return ((fa<=5.0L && fc<=5.0L)? 1: 2);     /* bracket already found */
  
  *ax=b0; *bx=b0+distx/2.0L; *cx=b0+distx;         /* try search from center of bracket */
  fa = (*func)(*ax, par1, par2, par3); 
  fb = (*func)(*bx, par1, par2, par3);
  fc = (*func)(*cx, par1, par2, par3);
  if (fa>fb && fc>fb) return ((fa<=5.0L && fc<=5.0L)? 1: 2);     /* bracket inside start interval */
  ...
  *cx = (*bx) + GOLD*(*bx-*ax);
  ...
  for (i=1;i<=NTRY;i++) {
      ...
      if (fb < fc && (c0-*cx)*(*cx-*bx)>0.0L) return ((fa<=5.0L && fc<=5.0L)? 1: 2);
      if ((c0-*cx)*(*cx-*bx)<=0.0L) {
	  *cx=c0; 
	  fc = (*func)(*cx, par1, par2, par3);
	  if (fb < fc) return ((fa <=5.0L && fc<=5.0L)? 1: 2);
      }
