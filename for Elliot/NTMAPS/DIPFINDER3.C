...
long double pi;
long double a0, a00, ax, ax0, axx, da, b0, bx, bxx, xacc, yacc, bracs, mfac, comptol;
...
int shp=0, root_no, bcfno, brakno, sheqbc=1, rootexit=1, rootexit2=1;
unsigned long int qq, pp;
char shinofilename[160], shinofilenamebase[60], shinofilenameend[60], compfilename[60];
char bifcfilename[100][60];
...
main(int argc, char **argv)
{
    ...
    int i, oldpt=1, errorcount=1;
    
    /* read input */
    makeinitials();
    fl3=fopen(compfilename, "w");
    fprintf(fl3,"# %18.15s %20.15s %7s %7s %12s %5s\n\n", 
	    "a", "b", "q", "p", "b-bifcb", "shp");
    fclose(fl3);
...
void makeinitials(void)      /* read and adjust initial values */
{
  ...
  fscanf(fl,"%Lf", &ax0); NL(fl, dummy);      /* amax */
  ...
  fscanf(fl,"%d", &bcfno); NL(fl, dummy);     /* number of bifc input files */

  for (i=0;i<bcfno;i++) {
      fscanf(fl,"%s", &bifcfilename[i][0]); NL(fl, dummy);  /* bifc input filename */
  }
...
void prepoutfiles(int bc)
{
    char dummy, ctmp[40];

    fl2=fopen(&bifcfilename[bc][0], "r");
    ...
    /* adjust initials */
    if (a00<=((long double)(qq))/((long double)(pp))) {
	a0=((long double)(qq))/((long double)(pp));
	b0=0.0L;
    }
    else a0=a00;
    if (b0<=0.0L) b0=0.0L;  
    if (ax0<a0) ax=a0; else ax=ax0;
...
void result(void)
{
	...
	if (a<=axx && rootexit>0 && rootexit2>0) {
		...
		if (fabs(b-bifcb)<=comptol && fabs(b2-bifcb)<=comptol) sheqbc=1;
			...
			if (sheqbc==1)
			    fprintf(fl3,"%20.15Lf %20.15Lf %7u %7u %12.8Le %5d\n", 
				    a, b, qq, pp, b-bifcb, shp);
			sheqbc=0;
		    }
		}
	    }
