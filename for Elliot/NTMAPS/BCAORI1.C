...
long double pi2;
long double omega, a0, a00, a0min, a0max, a0step, b0, b00, ymin, ymax, ystep, y00, bxx, bracfac;
...
void makeinitials(void)      /* read and adjust initial values */
{
  ...
  fscanf(fl,"%Lf", &bracfac);  NL(fl, dummy);          /* distance of first mn-brac-b */
...
void maincalc() 
{
    long double braca, bracb, bracc, solb[2*xbranch+1], soly[2*xbranch+1], checker, bracdist, tmp;
    int xcount, i, j;


    for (a0=a0min; a0<=a0max; a0+=a0step) {
      xcount=0;
      
      if (fabsl(a0-a0min)<SMALL) {
	braca=sqrtl(1.0L-omega/a0);
	if (rootbyfunc(braca, sln, qq[qpi], pp[qpi])>0.0L) braca=-braca;
      }
      else braca=soly[xcount/2];
      bracb=braca*bracfac; bracdist=braca-bracb;
      
      mnbrac(&braca, &bracb, &bracc, rootbyfunc, sln, qq[qpi], pp[qpi]);
      solb[xcount]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &soly[xcount]);
      
      (xcount)++;
      braca=soly[0]; bracb=braca-bracdist;
      mnbrac(&braca, &bracb, &bracc, rootbynegfunc, sln, qq[qpi], pp[qpi]);
      solb[xcount]=brent(braca, bracb, bracc, rootbynegfunc, sln, qq[qpi], pp[qpi], &soly[xcount]);
      checker=rootbyfunc(soly[xcount]-bracdist, sln, qq[qpi], pp[qpi]);
      if (checker>0.0L) (xcount)--;
      
      while (checker <0.0L && (xcount)<2*xbranch) {
	(xcount)++;
	braca=soly[xcount-1]; bracb=braca-bracdist;
	mnbrac(&braca, &bracb, &bracc, rootbyfunc, sln, qq[qpi], pp[qpi]);
	solb[xcount]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &soly[xcount]);
	...
	for (j=i+1; j<=xcount; j++) {
	  if (soly[i]<soly[j]) {tmp=soly[j]; soly[j]=soly[i];soly[i]=tmp;
	  tmp=solb[j]; solb[j]=solb[i];solb[i]=tmp;}
	}
      
      
      fprintf(fl1, "%25.18Lf%25.18Lf%25.18Lf%10d\n", a0, solb[xcount/2], soly[xcount/2], xcount+1);  
      for (i=1; i<=(xcount/2); i++) {
	fprintf(fl1, "%25.18Lf%25.18Lf%25.18Lf%10d\n", a0, solb[i+xcount/2], soly[i+xcount/2], xcount+1);  
	fprintf(fl1, "%25.18Lf%25.18Lf%25.18Lf%10d\n", a0, solb[-i+xcount/2], soly[-i+xcount/2], xcount+1);  
      }
    }
}
...
{
  long double GLIMIT=100.0L, EPS=LDBL_EPSILON, TINY=EPS;
