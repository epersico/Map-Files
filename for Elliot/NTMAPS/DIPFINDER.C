...
const char *infilename="dipfinder.in";
const long double SMALL=1.0e-10L;
const int SUPPERROR=15;

void makeinitials(void);
void prepoutfiles(void);
void result(FILE *,char[], long double, long double, int *, int, long double);
...
long double a, b, bifca, bifcb;

long double shinox, shinoyf, shinopx, shinopyf;
int shp, root_no, brakno, sheqbc=1;
...
main(int argc, char **argv)
{
    ...
    long double olda[2], oldb[2], oldm[2];
    int rootexit=1, oldpt=1, errorcount=1;
	...
	for (a=a0+da;a<=ax;a+=da)
	{
	    ...
	    rootexit=rootbybrak(&xrootfunc, &b, b, ((b+bx<bxx)? (b+bx): bxx), 
				brakno, root_no); 
	    result(fl1,"%20.15Lf%20.15Lf%5d%10d%20.10Lg\n", a, b, 
		   &sheqbc, rootexit, yrootfunc(b));
	...
	if (a<=((long double)(qq))/((long double)(pp))+SMALL) {b=0; rootexit=1;}
	else {
	    assignshino(a,shp);
	...
	oldpt=1-oldpt; olda[oldpt]=a-da; oldb[oldpt]=b;
...
void prepoutfiles(void)
{
    ...
    fprintf(fl1,"# %18s%20s%5s%10s%20s\n\n", "a", "b", "s=b","exitcode","yrootfct");
}

void result(FILE *fl, char formats[], long double aa, long double bb, 
	    int *sbc, int rtex, long double yrt)
{
    char dummy;
	...
	do {	    
		...
		fscanf(fl2,"%Le", &bifcb); 
		NL(fl2,dummy); 
		if (dummy==EOF) axx=aa-SMALL;		
	    }   
	    else axx=aa-SMALL;
	}
	while (aa-bifca>comptol && aa<=axx);
	if (fabs(aa-bifca)<=comptol){
	    if (fabs(bb-bifcb)<=comptol) *sbc=1;
	    else {
		if (*sbc==1)
		    fprintf(fl3,"%20.15Lf %20.15Lf %10u %10u %12.8Le\n", 
			    aa, bb, qq, pp, bb-bifcb);
		*sbc=0;	    
	    }
	}
	else *sbc=0;
    ...
    fprintf(fl,formats, aa, bb, *sbc, rtex, yrt);
}
