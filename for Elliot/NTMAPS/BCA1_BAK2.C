...
/*    long double braca, bracb, bracc, sol[10][2], nsol[10][2], sol[2*xbranch+2][2], nsol[2*xbranch+2][2], checker, bracd, mindist, intervals=INTERVALS; */
    ...
    long double braca, bracb, bracc, sol[IPOW3(xbranch)+1][2], nsol[IPOW3(xbranch)+1][2], 
	checker, inters=INTERVALS, boundl, boundu, tmp[2]; 
    int xcount, solno=IPOW3(xbranch), i, j, k, jold, okbrac;
	...
	if (rootbyfunc(bracb, sln, qq[qpi], pp[qpi])>=0.0L) {
	    printf("Warning: No valid initial guess found, aborting...\n");
	    exit(-10); }}}
	
	/* estimate bounds on domain */
	boundl=bracb-1.0L/bracdist; boundu=bracb+1.0L/bracdist;
	tmp[0]=boundl; tmp[1]=bracb; 
	if (rootbyfunc(boundl, sln, qq[qpi], pp[qpi])>=0.0L) {
	    for (i=1;i<=(-(int)(log(bracdist)/log(2.0L))+10);i++) {
		boundl=(tmp[0]+tmp[1])/2.0L;
		if (rootbyfunc(boundl, sln, qq[qpi], pp[qpi])>=0.0L) tmp[0]=boundl;
		else tmp[1]=boundl;
	    }
	    boundl=tmp[1];
	}
	
	tmp[0]=bracb; tmp[1]=boundu; 
	if (rootbyfunc(boundu, sln, qq[qpi], pp[qpi])>=0.0L) {
	    for (i=1;i<=(-(int)(log(bracdist)/log(2.0L))+10);i++) {
		boundu=(tmp[0]+tmp[1])/2.0L;
		if (rootbyfunc(boundu, sln, qq[qpi], pp[qpi])>=0.0L) tmp[1]=boundu;
		else tmp[0]=boundu;
	    }
	    boundu=tmp[0];
	}
	...
	/* find first maximum */
	braca=boundl; bracc=boundu;
	if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))==1)
	    nsol[0][1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], 
				   &nsol[0][0]);
	else { printf("Warning: No start maximum found, aborting...\n"); exit(-1); }

	if (rootbyfunc(nsol[0][0], sln, qq[qpi], pp[qpi])>=0.0L) {
	    printf("Warning: No start maximum found, aborting...\n");
	    exit(-2); }

	/* find higher branch maxima */
	i=1; while (i<=xbranch && xcount<solno-1){
	    j=0; while (j<xcount && xcount<solno){
		if (j>=2) braca=nsol[j-2][0]; else braca=boundl; 
		bracb=braca-1.0L; bracc=nsol[0][0];
		if((okbrac=mymnbrac(&braca, &bracb, &bracc, inters, rootbyfunc, sln, qq[qpi], pp[qpi]))==1)
		    tmp[1]=-brent(braca, bracb, bracc, rootbyfunc, sln, qq[qpi], pp[qpi], &tmp[0]);
		
		if (okbrac==1 && rootbyfunc(tmp[0], sln, qq[qpi], pp[qpi])<0.0L) {
		    for (k=j;k<xcount; k++) {nsol[k+2][0]=nsol[k][0]; nsol[k+2][1]=nsol[k][1];}
			...
			nsol[j+1][1]=brent(braca, bracb, bracc, rootbynegfunc, sln, qq[qpi], pp[qpi], 
					  &nsol[j+1][0]);
		    else printf("Warning: No minimum between start maxima...\n");
		}
	    j++;}
	i++;}
    }
